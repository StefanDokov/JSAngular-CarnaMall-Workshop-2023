...
export class DetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      ...
      this.infoForm = this.fb.group({
        ...
        costPrice: [
          { value: this.rent.price, disabled: true },
          Validators.required,
        ],
      });
    }, 1050);
  ...
  getRenta() {
    ...
    this.api.getOneRent(id).subscribe({
      next: (res) => {
        this.rent = res.resu;
      },
      error: (err) => {
  ...
  getUseri() {
    this.auth.getProfile().subscribe((res) => (this.useri = res));
  }
  isRentingClick() {
